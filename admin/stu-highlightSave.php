...
$name = $_POST["name"];
$stu_faculty = $_POST["faculty"];
$highlight = $_POST["highlight"];

if (isset($_POST['save'])) {
	if ($_FILES["filename"]["tmp_name"] == "") {
		$newfilename = "";
		$sql = "INSERT INTO `stu_highlight` (`id`, `name`, `faculty`, `highlight`, `filename`, `date_regis`) 
		VALUES (NULL, '$name', '$stu_faculty', '$highlight', '$newfilename', current_timestamp());";
		mysqli_query($conn, $sql);
		echo '<script language="javascript">';
		echo 'alert("บันทึกข้อมูลเรียบร้อยแล้ว"); location.href="stu-highlightAdd.php"';
		echo '</script>';
	}
	...
	if ($_FILES["filename"]["size"] > 500000) {
		echo '<script language="javascript">';
		echo 'alert("Sorry, your file is too large."); location.href="stu-highlightAdd.php"';
		echo '</script>';
		$uploadOk = 0;
	}
